...
/// Similar to `get_premise_term`, but extracts the term from a `ProofCommand`, not a `Premise`.
fn get_single_term_from_command_err(command: &ProofCommand) -> Result<&Rc<Term>, CheckerError> {
    super::get_single_term_from_command(&command)
...
pub fn bind(
    ...
    let previous_command = &subproof_commands[subproof_commands.len() - 2];
    let (phi, phi_prime) =
        match_term_err!((= p q) = get_single_term_from_command_err(previous_command)?)?;
...
pub fn r#let(
    ...
    // subproof
    let previous_term =
        get_single_term_from_command_err(&subproof_commands[subproof_commands.len() - 2])?;
