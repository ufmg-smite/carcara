...
pub fn parse_terms_with_pool<const N: usize>(
    ...
    let mut parser = Parser::new(pool, definitions.as_bytes(), true).expect(ERROR_MESSAGE);
...
pub fn parse_term_err(input: &str) -> Error {
    let mut pool = TermPool::new();
    Parser::new(&mut pool, input.as_bytes(), true)
...
pub fn parse_proof(input: &str) -> Proof {
    let mut pool = TermPool::new();
    let commands = Parser::new(&mut pool, input.as_bytes(), true)
...
fn test_hash_consing() {
    ...
    let mut parser = Parser::new(&mut pool, input.as_bytes(), true).unwrap();
