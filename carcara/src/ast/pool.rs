...
mod MultiThreadPool {
    ...
    /// A structure to store and manage all allocated terms.
    ///
    /// You can add a `Term` to the pool using [`MergedPool::add`], which will return an `Rc<Term>`. This
    /// struct ensures that, if two equal terms are added to a pool, they will be in the same
    /// allocation. This invariant allows terms to be safely compared and hashed by reference, instead
    /// of by value (see [`Rc`]).
    ///
    /// This struct also provides other utility methods, like computing the sort of a term (see
    /// [`MergedPool::sort`]) or its free variables (see [`MergedPool::free_vars`]).
    ...
    impl MergedPool {
        /// Construct a new `MergedPool` whose dynamic pool is empty and its
        /// constant pool is non existing.
        pub fn new() -> Self {
            Self {
        ...
        /// Instantiates a new `MergedPool` from a previous single thread term pool
    ...
    impl TPool for MergedPool {
        ...
        /// Takes a vector of terms and calls [`MergedPool::add`] on each.
        fn add_all(&mut self, terms: Vec<Term>) -> Vec<Rc<Term>> {
            terms.into_iter().map(|t| self.add(t)).collect()
        }
