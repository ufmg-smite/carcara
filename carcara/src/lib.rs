...
fn extract_step(sliced_step: &ProofCommand) -> &ProofStep {
    match sliced_step {
        ...
        ProofCommand::Assume { .. } => panic!("Tried to extract step from assume."), // But this shouldn't happen since extract step is only used on things we already know aren't assumes
    }
}
