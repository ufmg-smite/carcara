...
fn arith_poly_norm() {
    test_cases! {
        ...
        ",
        "Simple working examples" {
            "(step t1 (cl (= (+ (* 2 k) (* 1 n)) (+ n (* k 2)))) :rule arith_poly_norm)": true,
            "(step t1 (cl (=
                (+ (* 2.0 y) (* 1.0 x))
                (+ x (* y 2.0))
            )) :rule arith_poly_norm)": true,
        }
        "Coefficient cancellation" {
            ...
            "(step t1 (cl (= (+ 2 (- 1) (- 1)) 0)) :rule arith_poly_norm)": true,
            "(step t1 (cl (= (* 0.0 x) 0.0)) :rule arith_poly_norm)": true,
        }
        "Failing examples" {
            "(step t1 (cl (= (+ k k) (+ k 0))) :rule arith_poly_norm)": false,
            "(step t1 (cl (= (* 2.0 x) (+ 2.0 x))) :rule arith_poly_norm)": false,
        }
    }
}
