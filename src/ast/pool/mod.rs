...
impl Pool {
    ...
    pub fn compute_free_vars<'a>(&'a mut self, term: &Rc<Term>) -> &'a IndexSet<Rc<Term>> {
        ...
        let set = match term.as_ref() {
            Term::App(f, args) => {
                let mut set = self.compute_free_vars(f).clone();
                for a in args {
            ...
            Term::Binder(_, bindings, inner) => {
                let mut vars = self.compute_free_vars(inner).clone();
            ...
            Term::Match(term, cases) => {
                let mut vars = self.compute_free_vars(term).clone();
                for case in cases {
                    let mut res_vars = self.compute_free_vars(&case.body).clone();
